...
function Badge({ ext, category }) {
  ...
  return (
    <span style={{
      ...
      border: `1px solid ${isMail ? 'rgba(90,240,232,0.2)' : 'rgba(232,255,90,0.2)'}`,
    }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-tertiary)', fontFamily: 'var(--mono)' }}>
            {formatSize(size)}
            {wasOLE2 && <span style={{ marginLeft: 10, color: 'rgba(232,255,90,0.5)' }}>· OLE2 deep scan</span>}
          </div>
        </div>
      ...
      <div>
        ...
        >
          Magic bytes (hex) <span style={{ fontSize: 9 }}>{hexExpanded ? '▲' : '▼'}</span>
        </div>
        <div style={{
